 ...
 * DS-024 Golpe del Nudo: "Un Campeón que controlas gana +2 de ATQ hasta el
 * final del turno. Si destruye a un Campeón que controla el rival este turno,
 * el rival pierde 1 carta de su mano al azar."
 *
 * Patrón D7: armado de pendiente → resolución con contextoUso.
 * Requisito de resolución: requiere ≥1 Campeón propio (FB-024) / ≥2 (DS-024).
 */
...
export function registrarEfectosCombate(): void {
  ...
  // DS-024 Golpe del Nudo: mismo efecto que FB-024 (+2 ATQ), condición distinta (≥2 campeones)
  registrarEfecto('al-resolver-cadena', 'DS-024', (s, _ctx, inst, payload) => {
    if (payload.contextoUso === 'objetivo-elegido') {
      aplicarMod(s, payload.objetivoId!, 'poder', 2, 'ocaso')
      return
    }
